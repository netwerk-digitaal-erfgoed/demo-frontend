...
podTemplate(label: 'jenkins-slave', containers: [
  ...
  volumes: [
    ...
    node('jenkins-slave') {
        ...
        git url: 'https://github.com/netwerk-digitaal-erfgoed/demo-frontend.git', branch: 'master'

        stage('Clone repository') {
            container('git') {
                sh 'git clone -b master  https://github.com/netwerk-digitaal-erfgoed/demo-frontend'
            }
        }
            ...
            container('docker') {
                dir('demo-frontend/') {
                    withVault([configuration: configuration, vaultSecrets: secrets]) {
                        ...
                        sh 'docker build . -t harbor.51-105-200-91.nip.io/nde/demo-frontend'
                        sh 'docker push harbor.51-105-200-91.nip.io/nde/demo-frontend'
                    }
                }
            }
        }
        ...
        stage('Deploy to kubernetes') {
            container('kubectl') {
                dir('demo-frontend/') {
                    ...
                    sh 'kubectl delete pod -l app=nde-frontend'
                }
            }
        }
    }
